...
export const search = async function async(query, filter) {
  return new Promise((resolve, reject) => {
    ytmusic.search(query, filter).then((response) => {
      resolve(response)
      response.filter((result) => {return result.resultType === 'song' || result.resultType === 'video'}).forEach((result) => {
